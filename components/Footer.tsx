"use client"

import { Zap, Mail, Globe } from "lucide-react"

const NAV_LINKS = [
  { label: "Soluções", href: "#solucoes" },
  { label: "Diferenciais", href: "#diferenciais" },
  { label: "Processo", href: "#processo" },
  { label: "Projetos", href: "#projetos" },
  { label: "FAQ", href: "#faq" },
  { label: "Contato", href: "#contato" },
]

function InstagramIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
      <circle cx="12" cy="12" r="4" />
      <circle cx="17.5" cy="6.5" r="0.5" fill="currentColor" stroke="none" />
    </svg>
  )
}

const SOCIAL = [
  { icon: InstagramIcon, label: "Instagram", href: "https://www.instagram.com/garbatosolution/" },
]

export default function Footer() {
  return (
    <footer
      className="relative border-t"
      style={{ borderColor: "var(--gs-border)", background: "#0A0E1A" }}
    >
      {/* Top gradient */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#3B82F6]/30 to-transparent" />

      <div className="max-w-7xl mx-auto px-6 py-14">
        <div className="grid lg:grid-cols-4 gap-10 mb-12">
          {/* Brand */}
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#3B6EF6] to-[#2FE0D0] flex items-center justify-center shadow-[0_4px_14px_rgba(59,130,246,0.25)]">
                <Zap className="w-4 h-4 text-white" strokeWidth={2.5} />
              </div>
              <span
                className="text-[15px] font-semibold text-[#EEF1F8]"
                style={{ fontFamily: "var(--font-space-grotesk)" }}
              >
                Garbato<span className="text-[#2FE0D0]"> Solution</span>
              </span>
            </div>

            <p className="text-[#AFB6CC] text-[14px] leading-relaxed max-w-xs mb-6">
              Desenvolvemos sistemas sob medida para empresas que precisam
              organizar processos, centralizar informações e ganhar
              produtividade com tecnologia.
            </p>

            {/* Contact info */}
            <div className="flex flex-col gap-2">
              <a
                href="mailto:contato@garbatosolution.com.br"
                className="flex items-center gap-2 text-[13px] text-[#AFB6CC] hover:text-[#EEF1F8] transition-colors"
              >
                <Mail className="w-3.5 h-3.5 text-[#3B82F6]" />
                contato@garbatosolution.com.br
              </a>
              <div className="flex items-center gap-2 text-[13px] text-[#AFB6CC]">
                <Globe className="w-3.5 h-3.5 text-[#3B82F6]" />
                garbatosolution.com.br
              </div>
            </div>
          </div>

          {/* Navigation */}
          <div>
            <div
              className="text-[11px] font-bold text-[#71789A] uppercase tracking-wider mb-4"
              style={{ fontFamily: "var(--font-space-grotesk)" }}
            >
              Navegação
            </div>
            <ul className="flex flex-col gap-2.5">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-[13.5px] text-[#AFB6CC] hover:text-[#EEF1F8] transition-colors"
                    onClick={(e) => {
                      e.preventDefault()
                      document
                        .querySelector(link.href)
                        ?.scrollIntoView({ behavior: "smooth" })
                    }}
                  >
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Social + team */}
          <div>
            <div
              className="text-[11px] font-bold text-[#71789A] uppercase tracking-wider mb-4"
              style={{ fontFamily: "var(--font-space-grotesk)" }}
            >
              Redes sociais
            </div>
            <div className="flex flex-wrap gap-2 mb-6">
              {SOCIAL.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={s.label}
                  className="w-9 h-9 rounded-lg flex items-center justify-center transition-all hover:-translate-y-0.5"
                  style={{
                    background: "#0E1320",
                    border: "1px solid var(--gs-border)",
                    boxShadow: "var(--gs-shadow-sm)",
                  }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.borderColor = "rgba(59,130,246,0.30)"
                    e.currentTarget.style.background = "rgba(59,130,246,0.05)"
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.borderColor = "var(--gs-border)"
                    e.currentTarget.style.background = "#0E1320"
                  }}
                >
                  <s.icon className="w-4 h-4 text-[#AFB6CC]" />
                </a>
              ))}
            </div>

            {/* Team member */}
            <div
              className="rounded-xl p-3 flex items-center gap-3"
              style={{
                background: "#0E1320",
                border: "1px solid var(--gs-border)",
                boxShadow: "var(--gs-shadow-sm)",
              }}
            >
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#3B6EF6] to-[#2FE0D0] flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0">
                JG
              </div>
              <div>
                <div className="text-[13px] font-semibold text-[#EEF1F8]">
                  Juan Garbato
                </div>
                <div className="text-[11px] text-[#71789A]">CEO · Fundador</div>
              </div>
            </div>
          </div>
        </div>

        {/* Bottom bar */}
        <div
          className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-3"
          style={{ borderTop: "1px solid var(--gs-border)" }}
        >
          <p className="text-[12px] text-[#71789A]">
            © {new Date().getFullYear()} Garbato Solution. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-4">
            <a href="#" className="text-[12px] text-[#71789A] hover:text-[#AFB6CC] transition-colors">
              Política de Privacidade
            </a>
            <a href="#" className="text-[12px] text-[#71789A] hover:text-[#AFB6CC] transition-colors">
              Termos de Uso
            </a>
          </div>
        </div>
      </div>
    </footer>
  )
}
